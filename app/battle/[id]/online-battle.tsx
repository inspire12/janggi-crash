...
export default function OnlineBattle({ matchId }: { matchId: string }) {
  ...
  async function resign() {
    if (data?.match.status === 'active') setResignOpen(true);
  }
  async function confirmResign() {
    if (!data || data.match.status !== 'active' || sending) return;
    ...
    if (!response.ok) throw new Error('기권 요청을 처리하지 못했습니다. 다시 시도해 주세요.');
    setResignOpen(false);
    await refresh();
    } catch (cause) { setResignOpen(false); setError(cause instanceof Error ? cause.message : '연결을 확인해 주세요.'); }
  ...
  return (
    <main className="game-shell online-shell">
      <ResignDialog open={resignOpen && data.match.status === 'active'} onOpenChange={setResignOpen} onConfirm={()=>void confirmResign()} busy={sending} online />
      ...
      <section className="online-match-layout">
        ...
        <div className="arena online-arena">
          ...
          <div className="clock-action-dock">
            <CompactClock label="상대" timeMs={clock(data.you.side === 'cho' ? 'han' : 'cho')} periodMs={data.match.periodMs} active={!result && data.match.turn !== data.you.side} />
          {data.match.status === 'active' && <div className="battle-tools"><button title={data.match.opponentAllowsTakeback ? '상대에게 직전 수 무르기를 요청합니다.' : '상대가 무르기 요청을 받지 않습니다.'} disabled={sending || !data.match.canTakeback || data.match.takebackRequested || !data.match.opponentAllowsTakeback || Boolean(trialBoard)} onClick={() => void matchAction('takeback-request')}><Undo2 /> {data.match.opponentAllowsTakeback ? '무르기 요청' : '무르기 거부 중'}</button><button className={trialBoard ? 'active' : ''} disabled={sending || data.match.takebackRequested} onClick={toggleTrial}><FlaskConical /> {trialBoard ? '둬보기 종료' : '둬보기'}</button><button className="danger" disabled={sending || Boolean(trialBoard)} onClick={() => void resign()}><Flag /> 기권</button></div>}
            <CompactClock label="나" timeMs={clock(data.you.side)} periodMs={data.match.periodMs} active={!result && data.match.turn === data.you.side} />
          </div>
        </div>
...
function RankUpdate({before,after}:{before:number;after:number}) {
  const oldRank=rankForScore(before),newRank=rankForScore(after),delta=after-before;
  const changed=oldRank.key!==newRank.key;
  return <section className="rank-update rank-update-compact" aria-label="승강급 결과"><div><span>{changed?`${oldRank.name} → ${newRank.name}`:newRank.name}{changed && <small> · {after>before?'승급':'강급'}</small>}</span><strong>{delta>0?'+':''}{delta}점</strong></div><p>{newRank.next===null?'최고 등급':`승급까지 ${newRank.next}점`}</p></section>;
}

function CompactClock({label,timeMs,periodMs,active}:{label:string;timeMs:number;periodMs:number;active:boolean}) {
  const byoyomi=periodMs>0 && timeMs<=periodMs;
  return <div className={`compact-clock ${active?'running':''} ${byoyomi?'byoyomi':''}`} aria-label={`${label} ${byoyomi?'초읽기':'생각시간'}`}><span>{label}</span><strong>{formatClock(byoyomi?timeMs:Math.max(0,timeMs-periodMs))}</strong><small>{byoyomi?'초읽기':'생각시간'}</small></div>;
}
