...
export default function PracticePage() {
  ...
  function resign() {
    if (!winner && !motion) setResignOpen(true);
  }
  ...
  return (
    ...
    >
      <ResignDialog open={resignOpen && !winner} onOpenChange={setResignOpen} onConfirm={confirmResign} />
      <FormationDialog
        open={setupOpen}
        cho={choFormation}
        han={hanFormation}
      ...
      <fieldset className="game-action-tabs" aria-label="대국 조작">
        <button onClick={reset}><RotateCcw /><span>재대국</span></button>
        <button disabled={history.length === 0 || Boolean(trialSnapshot)} onClick={takeback}><Undo2 /><span>무르기</span></button>
        <button aria-pressed={Boolean(trialSnapshot)} className={trialSnapshot ? 'active' : ''} disabled={Boolean(winner)} onClick={toggleTrial}><FlaskConical /><span>{trialSnapshot ? '둬보기 종료' : '둬보기'}</span></button>
        <button className="danger" disabled={Boolean(winner)} onClick={resign}><Flag /><span>기권</span></button>
      </fieldset>
      <MatchInfo board={trialSnapshot?.pieces ?? pieces} moves={history.length} />
      <footer>
        <span>
